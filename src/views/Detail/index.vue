  ...
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="conPoin">
        ...
        <span v-show="categoryView.category3Name">{{ categoryView.category2Name }}</span>
...
export default {
  ...
  data() {
    return {
      list: ['手机社区', '规格与包装', '守候保障', '商品评价', '商品价格'],
      currIndex: 0,
      skuNum: 1, // 购买产品的个数
    }
  },
  methods: {
    ...
    async addShopCart() {
      try {
        ...
        // 2.成功 路由跳转(传递参数) 返回的是 ok 需要携带信息
        // 需要传递个数(skuNum) 和 产品的数据(skuInfo) 如果是query传参 地址栏会很有很长的乱码
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo)) // 存储到本地存储中解决上方的问题(本地存储只能存储字符串)
  ...
  mounted() {
    this.$store.dispatch('getCoodInfo', this.$route.params.skuid)
  },
  components: {
    ImageList,
    Zoom,
