...
export default {
  ...
  methods: {
    // 获取支付信息
    async getPayInfo() {
      const result = await this.$http.reqPayInfo(this.orderId)
      if (result.code == 200) {
    ...
    async open() {
      ...
      // 扫码成功直接跳转的逻辑 上方是点击了 已支付成功的逻辑
      // 需要知道支付成功或者失败(弹出窗弹出需要一直向服务器请求,看有没有支付)
      // 如果没有定时器 就开启一个
      if (!this.timer) {
